...
cdef class Part:
    """One part of a multipart/form-data request (Sync)
    """
...
cdef class AsyncPart:
    """One part of a multipart/form-data request (Async)
    """
...
cdef class _AsyncParser:
    ...
    def __init__(self, bytes delimiter, bytes ender, bint strict):
        ...
        self.expected_parts = []
        self.active_part = None
        self.default_part = AsyncPart('_default', AsyncNullTarget())
        self._leftover_buffer = None
        self.strict = strict
        self.unexpected_part_name = ''
    ...
    def register(self, str name, object target):
        ...
        if part:
            ...
            self.expected_parts.append(AsyncPart(name, target))

    async def set_active_part(self, part, str filename):
        self.active_part = part
        self.active_part.set_multipart_filename(filename)
        await self.active_part.start()

    async def unset_active_part(self):
        if self.active_part:
            ...
            if self.state == ParserState.PS_START:
                ...
                if 'content-disposition' in message:
                    ...
                    if name:
                        ...
                        await self.set_active_part(part, params.get('filename'))
